...
/////////////////////////////////////////////////////////
    ...
    element_mul_zn(u,temp6,r);//步骤2
	element_printf("u = %B\n", u);
    element_from_hash(h, "H1mUT", 5);//步骤3
	element_printf("h = %B\n", h);
    element_add(temp3, r, h);
	element_mul_zn(v, Sk, temp3);//步骤4
	element_printf("v = %B\n", v);
	pairing_apply(w, v, Pk, pairing);//步骤5
	element_printf("w = %B\n", w);
	time2 = get_time();
	...
	element_printf("hr = %B\n", hr);
    pairing_apply(temp7, u, Ss, pairing);
	...
	{
		printf("the authenticator of message is valid11111\n");
	}
    else if(element_cmp(wr, w))//双线性验证签名
	{
		printf("the authenticator of message is valid222222\n");
	}
	else
	{
